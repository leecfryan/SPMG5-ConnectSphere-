...
async function setup(t, getUser) {
  ...
  let reads = 0,
    recordChecks = 0;
  app.all(
    "/api/internal/test-venues/:id",
    requirePermission("venues.read"),
    (req, res) => {
      reads++;
      res.json({
        id: req.params.id,
        location: "Different venue location",
        capacity: 120,
      });
    },
  );
  app.get(
    "/api/internal/test-equipment/:type",
    ...
    app.get(
      ...
      (req, res) => {
        ...
        res.json({
          ...
          permission,
          information: "Restricted fixture",
        });
      },
    );
  }
...
const matrix = [
  { role: "venue_staff", permitted: ["venues.read", "bookings.read"] },
  {
    ...
    permitted: ["equipment.read", "technical_requests.read"],
  },
  {
...
for (const { role, permitted } of matrix) {
  test("Staff story AC1/2/4: permission matrix for " + role, async (t) => {
    ...
    for (const [permission, route] of Object.entries(paths)) {
      ...
      assert.equal(
        response.status,
        permitted.includes(permission) ? 200 : 403,
        permission,
      );
      if (!permitted.includes(permission)) {
        assert.deepEqual(
          counts(),
          before,
          "Denied access must not reach record lookup or data handler",
        );
        assert.doesNotMatch(
          await response.text(),
          /Restricted fixture|capacity|available/,
        );
      }
    }
  });
...
test("Staff story AC3: staff access is not restricted by venue, equipment type, or location", async (t) => {
  const { base } = await setup(t, async () =>
    authUser(["venue_staff", "technical_support_staff"], {
  ...
  for (const route of [
    "/api/internal/test-venues/unassigned-venue",
    "/api/internal/test-equipment/microphone",
  ]) {
    assert.equal(
      (await fetch(base + route, { headers: authHeaders })).status,
      200,
    );
  }
});
...
test("Staff story AC4: user metadata, headers, and query fields cannot elevate external users", async (t) => {
  const { base, counts } = await setup(t, async () =>
    authUser(["attendee"], {
      roles: ["venue_staff"],
  ...
  const response = await fetch(
    base + paths["venues.read"] + "?role=venue_staff&user_type=internal",
    {
      headers: { ...authHeaders, "X-User-Role": "venue_staff" },
    },
  );
  assert.equal(response.status, 403);
  assert.deepEqual(counts(), { reads: 0, recordChecks: 0 });
});
test("Staff story AC4: unknown/missing/malformed roles default to no internal access", async (t) => {
  for (const roles of [undefined, [], ["administrator"], "venue_staff"]) {
    ...
    assert.equal(
      (await fetch(base + "/api/internal/access", { headers: authHeaders }))
        .status,
      403,
    );
  }
...
test("Staff access endpoint returns only the current user's responsibilities", async (t) => {
  const { base } = await setup(t, async () =>
    authUser(["venue_staff", "attendee"]),
  );
  const response = await fetch(base + "/api/internal/access", {
    headers: authHeaders,
  });
...
test("Event-specific permissions require an explicit record-access resolver", () => {
  ...
  assert.throws(
    () => requirePermission("clients.read", true),
    /record access check/,
  );
...
test("Staff story AC4: unrelated/missing records and truthy non-boolean checks deny access", async (t) => {
  const { base, counts } = await setup(t, async () =>
    authUser(["event_coordinator"]),
  );
  for (const id of ["unrelated-event", "missing-event", "truthy"]) {
    const response = await fetch(
      base + "/api/internal/test-record/attendees.read/" + id,
      { headers: authHeaders },
    );
    assert.equal(response.status, 403);
  }
...
test("Staff story AC4: failed relationship lookup hides data and private errors", async (t) => {
  ...
  const response = await fetch(
    base + "/api/internal/test-record/clients.read/provider-down",
    { headers: authHeaders },
  );
...
test("Read permissions do not allow write requests", async (t) => {
  const { base, counts } = await setup(t, async () =>
    authUser(["venue_staff"]),
  );
  for (const method of ["POST", "PUT", "PATCH", "DELETE"]) {
    const response = await fetch(base + paths["venues.read"], {
      method,
      headers: authHeaders,
    });
    assert.equal(response.status, 403);
...
test("Role removal takes effect on the next verified request, using the same token", async (t) => {
  ...
  const { base } = await setup(t, async () => authUser(roles));
  assert.equal(
    (await fetch(base + paths["venues.read"], { headers: authHeaders })).status,
    200,
  );
  roles = ["attendee"];
  assert.equal(
