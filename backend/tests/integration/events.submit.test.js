...
beforeEach(() => {
  mock.restoreAll();
  createSubmitted = mock.method(
    repository,
    "createSubmitted",
    async (fields, organiserId) => ({
      id: "evt-1",
      status: "SUBMITTED",
...
test("SCRUM-51: 201 - a complete request comes back SUBMITTED", async () => {
  ...
  assert.equal(body.event.status, "SUBMITTED");
  assert.equal(body.event.organiser_id, DEV_ORGANISER_ID);
});
...
test("SCRUM-46/47: 201 - requirements fields are stored and returned", async () => {
  ...
  // Assert
  assert.equal(status, 201);
  assert.equal(body.event.venue_requirements, "Main hall, theatre layout for 200.");
  assert.equal(body.event.accessibility_needs, "Step-free access to the stage.");
  assert.equal(body.event.equipment_needs, "Projector and two radio mics.");
  assert.equal(body.event.other_comments, "Catering arrives an hour before doors.");
});
...
test("SCRUM-23: 400 - an empty body names every required field", async () => {
  ...
  assert.deepEqual(fieldsOf(body), [
    "description",
    "end_time",
    "expected_attendance",
    "name",
...
test("SCRUM-44: 400 - an over-long description is rejected", async () => {
  ...
  assert.equal(status, 400);
  assert.deepEqual(fieldsOf(body), ["description"]);
});
...
test("SCRUM-25 design: 500 - a database failure returns a generic message, not the Supabase error", async () => {
  ...
  assert.equal(body.error, "Could not submit the event request. Please try again.");
...
test("SCRUM-45 boundary: 201 - attendance of 1 is accepted over HTTP", async () => {
  ...
  assert.equal(status, 201);
  assert.equal(body.event.expected_attendance, 1);
});
...
test("SCRUM-45 boundary: 400 - attendance of 0 is rejected over HTTP", async () => {
  ...
  assert.equal(status, 400);
  assert.deepEqual(fieldsOf(body), ["expected_attendance"]);
  assert.equal(createSubmitted.mock.callCount(), 0);
});
...
test("SCRUM-44 boundary: 201 - a description of exactly 2000 characters is accepted", async () => {
  ...
  assert.equal(status, 201);
  assert.equal(body.event.description.length, 2000);
});
...
test("SCRUM-44 boundary: 400 - a name of 201 characters is rejected", async () => {
  ...
  assert.equal(status, 400);
  assert.deepEqual(fieldsOf(body), ["name"]);
  assert.equal(createSubmitted.mock.callCount(), 0);
});
...
test("SCRUM-45 boundary: 400 - a start_time in the past is rejected", async () => {
  ...
  assert.equal(status, 400);
  assert.deepEqual(fieldsOf(body), ["start_time"]);
  assert.equal(createSubmitted.mock.callCount(), 0);
});
