const internalRoles = new Set(["event_coordinator", "venue_staff", "technical_support_staff"]);
...
function requireAuth(authClient) {
  return async (req, res, next) => {
    ...
    try {
      ...
      if (error) {
        const unavailable = !error.status || error.status >= 500 || error.status === 429;
        return res.status(unavailable ? 503 : 401).json({
          message: unavailable ? "Unable to verify your session. Please try again." : "Your session has expired. Please sign in again.",
        });
      }
      if (!data?.user?.id) return res.status(401).json({ message: "Please sign in to continue." });
      const user = data.user;
      const rawRoles = user.app_metadata?.roles;
      const roles = Array.isArray(rawRoles)
        ? [...new Set(rawRoles.filter((role) => internalRoles.has(role) || externalRoles.has(role)))]
        : [];
      const accountTypes = [];
      if (roles.some((role) => internalRoles.has(role))) accountTypes.push("internal");
      if (roles.some((role) => externalRoles.has(role))) accountTypes.push("external");
