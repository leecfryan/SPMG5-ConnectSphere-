...
async function seedUsers() {
  ...
  if (!password || password.length < 16) {
    throw new Error(
      "Set SEED_USER_PASSWORD to at least 16 characters in the root .env.",
    );
  }
  ...
  for (const user of users) {
    const account = existing.get(user.email.toLowerCase());
    if (
      account &&
      ...
      throw new Error(
        "Existing account differs from this seed: " +
          user.email +
          ". Review it manually.",
      );
    }
