    ...
    fetch("/api/internal/access", {
      ...
      .then(async (response) => {
        if (response.status === 401 || response.status === 403) {
          throw new Error(
            "Your staff access is unavailable. Sign in again or contact your coordinator.",
          );
        }
        if (!response.ok)
          throw new Error(
            "We couldn’t load your responsibilities. Please try again.",
          );
      ...
      .catch((failure) => {
        if (!controller.signal.aborted)
          setError(failure.message || "Unable to load your responsibilities.");
      });
  ...
  return (
    ...
    >
      ...
      {error ? (
        ...
        <ul>
          {responsibilities.map(({ permission, label }) => (
            <li key={permission}>{label}</li>
          ))}
        </ul>
      ) : (
